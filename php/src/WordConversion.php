...
class WordConversion
{
    ...
    {
        return self::conversion([$word, null], 4);
    }
    ...
    public static function hex2bin($hex)
    {
        ...
        foreach ($hex_array as $number) {
            $bin_number = (string) (base_convert($number, 16, 2));
    ...
    public static function bin2hex($bin)
    {
        ...
        foreach ($bin_array as $number) {
            $hex .= (string) (base_convert($number, 2, 16));
        }
                ...
                if ('0' === $prevent || '0' === $current) {
                    switch ($type) {
                        ...
                        case 3:
                            // add
                        case 5:
                            return '0' == $prevent ? $current : $prevent;
                ...
                for ($i = $longest - 1; $i >= 0; --$i) {
                    ...
                    switch ($type) {
                        ...
                        case 3:
                            $value[$i] = $i > $shortest
                                ? 1
                                : ((int) $prevent_number ^ (int) ($current[$i]));

                            break;
                        // 非（按位取反）
                        case 4:
                            $value[$i] = '1' === $prevent_number
                                ? '0'
                                : '1';
